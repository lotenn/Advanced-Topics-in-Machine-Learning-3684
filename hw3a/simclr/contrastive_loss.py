...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = torch.dot(z_i, z_j) / (torch.norm(z_i) * torch.norm(z_j))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for z, z_N, i in [
          ...
          numerator = np.exp(sim(z, z_N) / tau)
          denominator = np.sum([np.exp(sim(z, out[m]) / tau) for m in range(2*N) if m != i])
          l.append(- np.log(numerator / denominator))
...
def sim_positive_pairs(out_left, out_right):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    similarity = torch.linalg.diagonal(torch.linalg.matmul(out_left, out_right.T))
    norm = torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1)
    pos_pairs = similarity / norm
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def compute_sim_matrix(out):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    similarity = torch.linalg.matmul(out, out.T)
    norm = torch.ger(torch.linalg.norm(out, dim=1), torch.linalg.norm(out.T, dim=0))
    sim_matrix = similarity / norm
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_positive_pairs_vector = sim_positive_pairs(out_left, out_right)
    sim_positive_pairs_vector = torch.cat([sim_positive_pairs_vector, sim_positive_pairs_vector], dim=0)
    ...
    numerator = torch.exp(sim_positive_pairs_vector / tau)
